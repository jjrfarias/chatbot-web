...
function ActionColumn({ title, tasks, tone, onComplete, onContact }: { title: string; tasks: CrmTask[]; tone?: "danger" | "dark"; onComplete: (task: CrmTask) => void; onContact: (task: CrmTask) => void }) {
  return <section className="rounded-2xl border border-cr-border bg-white"><div className="flex items-center justify-between border-b border-cr-border px-4 py-3"><span className="text-[12px] font-bold">{title}</span><span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${tone === "danger" ? "bg-red-50 text-red-600" : tone === "dark" ? "bg-cr-ink text-white" : "bg-cr-chip text-cr-muted"}`}>{tasks.length}</span></div><div className="p-2.5">{tasks.map((task) => <div key={task.id} className="mb-2 rounded-xl border border-cr-border-light p-3 last:mb-0"><div className="text-[12px] font-bold">{task.title}</div><div className="mt-1 text-[10.5px] text-cr-muted">{task.customer?.name} · {formatDate(task.dueAt)}</div><div className="mt-3 flex gap-2"><button onClick={() => onComplete(task)} className="rounded-lg border border-cr-border px-2.5 py-1.5 text-[10.5px] font-bold">Concluir</button>{task.customer?.phone && <button onClick={() => onContact(task)} className="rounded-lg bg-[#25D366] px-2.5 py-1.5 text-[10.5px] font-bold text-white">WhatsApp</button>}</div></div>)}{tasks.length === 0 && <div className="px-2 py-8 text-center text-[11px] text-cr-muted">Nenhuma ação</div>}</div></section>;
}
function Metric({ label, value, dark }: { label: string; value: number; dark?: boolean }) { return <div className={`rounded-2xl border p-4 ${dark ? "border-cr-ink bg-cr-ink text-white" : "border-cr-border bg-white"}`}><div className={`text-[10.5px] font-bold uppercase ${dark ? "text-cr-sidebar-muted" : "text-cr-muted"}`}>{label}</div><div className="mt-1 font-display text-xl font-bold">{value}</div></div>; }
