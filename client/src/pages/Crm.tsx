...
export function Crm() {
  ...
  if (!board) return <div className="px-11 py-8 text-sm text-cr-muted">Carregando CRM...</div>;
  ...
  return <div className="min-w-0 px-8 py-7">
    <div className="flex items-start justify-between"><div><h1 className="m-0 font-display text-2xl font-bold">CRM</h1><p className="mt-1 text-[12.5px] text-cr-muted">Acompanhe vendas e serviços em um só lugar</p></div><button onClick={() => setShowForm(true)} className="rounded-xl bg-cr-ink px-5 py-3 text-[13px] font-bold text-white">+ Nova oportunidade</button></div>
    <div className="mt-5 grid grid-cols-3 gap-3"><Metric label="Pipeline aberto" value={formatCurrency(totalPipeline)} /><Metric label="Oportunidades" value={String(board.opportunities.length)} /><Metric label="Tarefas atrasadas" value={String(overdue)} alert={overdue > 0} /></div>

    <div className="mt-5 inline-flex rounded-xl border border-cr-border bg-white p-1"><button onClick={() => setPipeline("vendas")} className={`rounded-lg px-4 py-2 text-[12px] font-bold ${pipeline === "vendas" ? "bg-cr-ink text-white" : "text-cr-muted"}`}>Vendas</button><button onClick={() => setPipeline("assistencia")} className={`rounded-lg px-4 py-2 text-[12px] font-bold ${pipeline === "assistencia" ? "bg-cr-ink text-white" : "text-cr-muted"}`}>Assistência técnica</button></div>

    {board.opportunities.length > 0 && <div className="mt-5 flex items-center gap-2.5"><input value={search} onChange={(e) => setSearch(e.target.value)} className="input max-w-sm bg-white" placeholder="Buscar cliente, telefone ou oportunidade" /><select value={staffFilter} onChange={(e) => setStaffFilter(e.target.value)} className="input max-w-56 bg-white"><option value="">Todos os responsáveis</option><option value="sem_responsavel">Sem responsável</option>{staff.map((person) => <option key={person.id} value={person.id}>{person.name}</option>)}</select>{(search || staffFilter) && <button onClick={() => { setSearch(""); setStaffFilter(""); }} className="px-2 text-[11.5px] font-semibold text-cr-muted">Limpar filtros</button>}<span className="ml-auto text-[10.5px] text-cr-muted">Arraste os cartões para mudar de etapa</span></div>}
...
function OpportunityModal({ opportunity, pipeline = "vendas", customers, staff, stages, onClose, onSaved }: { opportunity?: CrmOpportunity; pipeline?: "vendas" | "assistencia"; customers: CustomerSummary[]; staff: StaffUser[]; stages: { key: string; label: string }[]; onClose: () => void; onSaved: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/35 p-4" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}><form onSubmit={submit} className="w-full max-w-lg rounded-2xl bg-white p-5 shadow-xl"><div className="flex items-center justify-between"><h2 className="m-0 font-display text-lg font-bold">{opportunity ? "Editar oportunidade" : "Nova oportunidade"}</h2><button type="button" onClick={onClose} className="text-xl text-cr-muted">×</button></div><div className="mt-4 grid grid-cols-2 gap-3">
    ...
    <label className="col-span-2 text-xs font-semibold">Oportunidade<input required defaultValue={opportunity?.title} name="title" className="input mt-1" placeholder="Ex.: iPhone 15 Pro 256 GB" /></label>
  ...
  </div><div className="mt-5 flex justify-end gap-2"><button type="button" onClick={onClose} className="rounded-xl border border-cr-border px-4 py-2.5 text-xs font-bold">Cancelar</button><button disabled={saving} className="rounded-xl bg-cr-ink px-4 py-2.5 text-xs font-bold text-white disabled:opacity-50">{saving ? "Salvando..." : opportunity ? "Salvar alterações" : "Criar oportunidade"}</button></div></form></div>;
}
