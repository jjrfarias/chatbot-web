        ...
        {loading ? (
          ...
          <>
            <div className="flex items-center border-b border-cr-border px-[18px] py-2.5 text-[10.5px] font-bold uppercase text-cr-muted">
              <div className="hidden flex-1 md:block">Pedido</div>
              <div className="hidden flex-1 sm:block">Data</div>
              <div className="flex-[1.4]">Cliente</div>
              <div className="hidden flex-[1.8] sm:block">Aparelho</div>
              <div className="hidden flex-[1.6] lg:block">Troca</div>
              <div className="hidden flex-[1.2] md:block">Pagamento</div>
            ...
            {sales.map((sale) => (
              <div key={sale.id} className="flex items-center border-b border-cr-border-light px-[18px] py-3 last:border-0 hover:bg-cr-bg">
                <div className="hidden flex-1 text-xs text-cr-muted md:block">{sale.orderNumber}</div>
                <div className="hidden flex-1 text-xs text-cr-secondary sm:block">{formatDate(sale.createdAt)}</div>
                <div className="min-w-0 flex-[1.4] truncate text-[12.5px] font-semibold">{sale.customerName}</div>
                <div className="hidden flex-[1.8] truncate text-[12.5px] text-cr-secondary sm:block">
                  {sale.deviceName} · {sale.deviceColor}
                </div>
                <div className="hidden flex-[1.6] text-[12.5px] text-cr-secondary lg:block">
                  {sale.hasTradeIn ? `${sale.tradeInModelName} (${formatCurrency(sale.tradeInFinalValue)})` : "—"}
                </div>
                <div className="hidden flex-[1.2] text-[12.5px] text-cr-secondary md:block">{sale.paymentLabel}</div>
