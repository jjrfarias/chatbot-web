            ...
            <div className="flex flex-shrink-0 items-center gap-2">
              <div
                className={`flex h-[26px] w-[26px] flex-shrink-0 items-center justify-center rounded-full text-xs font-bold ${
                  done || active ? "bg-cr-accent text-white" : "border-[1.5px] border-cr-dot text-cr-muted"
                }`}
              >
                {done ? <CheckIcon className="h-3.5 w-3.5" /> : step}
              </div>
              <span className={`whitespace-nowrap text-[12.5px] ${active ? "font-bold text-cr-ink" : done ? "font-semibold text-cr-ink" : "font-medium text-cr-muted"}`}>
                {label}
              </span>
            </div>
            {step < STEPS.length && (
              <div className={`mx-2.5 h-px w-10 flex-shrink-0 ${done ? "bg-cr-accent" : "bg-cr-border"}`} />
            )}
          </div>
        );
      })}
