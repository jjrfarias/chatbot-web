...
repairsRouter.post("/", async (req, res) => {
  ...
  let customer = customerId ? await prisma.customer.findFirst({ where: { id: customerId, storeId } }) : null;
  if (!customer) customer = await prisma.customer.findFirst({ where: { name: customerName, storeId } });
  if (!customer && customerPhone) {
    customer = await prisma.customer.create({ data: { storeId, name: customerName, phone: customerPhone } });
  }
  ...
  await prisma.crmInteraction.create({
    data: { storeId, customerId: customer.id, type: "conserto", content: `Conserto de ${model} recebido e adicionado ao funil de assistência.` },
  });
...
repairsRouter.patch("/:id/status", async (req, res) => {
  ...
  const current = await prisma.repair.findFirst({ where: { id: req.params.id, storeId: req.storeId } });
  ...
  if (repair.crmOpportunityId) {
    ...
    if (repair.customerId) {
      await prisma.crmInteraction.create({
        data: { storeId: req.storeId, customerId: repair.customerId, type: "conserto", content: `Status do conserto de ${repair.model} atualizado para ${status}.` },
      });
    }
    if (status === "Concluído") {
